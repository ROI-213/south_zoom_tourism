...
const serviceImages: Record<string, { image: string; tag?: string }> = {
  "local-taxi": { image: serviceLocalTaxi, tag: "From ₹1,400 / 4 hrs" },
  ...
  "custom": { image: office1, tag: "Tailored Quote" },
};
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-4">
        ...
        <ul className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {servicesSection.items.map((item) => {
            ...
            return (
              <li key={item.id} className="group">
                <AppLink
                  href="/services"
                  className="flex h-full flex-col overflow-hidden rounded-2xl border border-border/80 bg-card shadow-sm transition-all duration-300 hover:-translate-y-2 hover:border-primary/60 hover:shadow-xl hover:shadow-primary/10"
                >
                  {/* Image header with live zoom animation and badges */}
                  <div className="relative h-36 w-full overflow-hidden bg-muted/40 sm:h-40">
                    <img
                      src={visual.image}
                      alt={item.title}
                      width={600}
                    ...
                    {/* Floating Icon badge */}
                    <span className="absolute left-3 top-3 grid h-9 w-9 place-items-center rounded-xl bg-background/90 text-primary shadow-md backdrop-blur-md transition-all duration-300 group-hover:scale-110 group-hover:bg-primary group-hover:text-primary-foreground">
                      <Icon className="h-4 w-4" aria-hidden="true" />
                    </span>

                    {/* Tag badge */}
                    {visual.tag ? (
                      <span className="absolute right-3 top-3 rounded-full bg-primary/95 px-2.5 py-0.5 text-[10px] font-bold text-primary-foreground shadow-sm backdrop-blur-sm">
                        {visual.tag}
                      </span>
                    ) : null}

                    {/* Title overlay on bottom of image */}
                    <h3 className="absolute bottom-3 left-3 right-3 text-sm font-bold text-white drop-shadow-sm flex items-center justify-between">
                      <span>{item.title}</span>
                      <ArrowUpRight className="h-4 w-4 text-white/80 opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
                    </h3>
                  </div>

                  {/* Card body content */}
                  <div className="flex flex-1 flex-col justify-between p-4">
                    <p className="text-xs text-muted-foreground line-clamp-3 leading-relaxed sm:text-sm">
                      {item.description}
                    </p>

                    <div className="mt-4 flex items-center gap-1 text-xs font-semibold text-primary transition-all group-hover:translate-x-1">
